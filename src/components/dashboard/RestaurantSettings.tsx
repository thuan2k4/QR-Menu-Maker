...
import { DEFAULT_MENU_TEMPLATE, MENU_TEMPLATES, getMenuTemplateById } from '../../constants/menuTemplates';
...
export default function RestaurantSettings({ user, restaurant, onCreated }: RestaurantSettingsProps) {
  ...
  const selectedTemplate = getMenuTemplateById(formData.templateId);
  const normalizeColor = (value: string | undefined) => (value || '').trim().toLowerCase();
  const isCustomTheme =
    normalizeColor(formData.primaryColor) !== normalizeColor(selectedTemplate.primaryColor) ||
    normalizeColor(formData.secondaryColor) !== normalizeColor(selectedTemplate.secondaryColor) ||
    formData.fontFamily !== selectedTemplate.fontFamily ||
    formData.sizePreset !== selectedTemplate.sizePreset ||
    formData.currency !== selectedTemplate.currency;
  ...
  const applyTemplatePreset = (templateId: string) => {
    const selectedTemplate = getMenuTemplateById(templateId);
    setFormData((prev) => ({
      ...prev,
      ...
      sizePreset: selectedTemplate.sizePreset,
      currency: selectedTemplate.currency,
      themeColor: selectedTemplate.primaryColor,
    }));
  };

  const resetCurrentTemplatePreset = () => {
    applyTemplatePreset(formData.templateId || DEFAULT_MENU_TEMPLATE.id);
  };
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <form id="settings-form" onSubmit={handleSubmit} className="p-8 space-y-8">
          <section className="rounded-3xl border border-gray-100 bg-gray-50/40 p-6">
            <div className="mb-6">
              <h3 className="text-lg font-bold text-gray-900">1. Thông tin cửa hàng</h3>
          ...
          <section className="rounded-3xl border border-gray-100 bg-white p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <h3 className="text-lg font-bold text-gray-900">2. Tùy chọn template và style</h3>
                <p className="text-sm text-gray-500">Chọn template trước, sau đó có thể tùy chỉnh sâu để tạo phong cách riêng.</p>
              </div>
              <span className="inline-flex px-3 py-1 rounded-full text-xs font-bold w-fit" style={{ backgroundColor: `${selectedTemplate.primaryColor}18`, color: selectedTemplate.primaryColor }}>
                Đang chọn: {selectedTemplate.name}
              </span>
            </div>

            <div className={`p-4 rounded-2xl border ${isCustomTheme ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200'}`}>
              {isCustomTheme ? (
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    <p className="text-sm font-bold text-amber-800">Bạn đang chỉnh tay trên template preset</p>
                    <p className="text-xs text-amber-700 mt-1">Template gốc: {selectedTemplate.name}. Các thay đổi màu/font/size/currency hiện ở chế độ custom.</p>
                  </div>
                  <button
                    type="button"
                    onClick={resetCurrentTemplatePreset}
                    className="px-4 py-2 rounded-xl text-sm font-bold bg-white border border-amber-300 text-amber-700 hover:bg-amber-100 w-fit"
                  >
                    Reset về preset
                  </button>
                </div>
                ...
                <div>
                  <p className="text-sm font-bold text-emerald-800">Đang dùng đúng preset của template</p>
                  <p className="text-xs text-emerald-700 mt-1">Bạn có thể chỉnh tay bên dưới để tạo bản custom riêng mà vẫn giữ template gốc để tham chiếu.</p>
                </div>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-bold text-gray-700">Template menu</label>
                <span className="text-xs text-gray-500">10 preset chủ đề</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {MENU_TEMPLATES.map((templateOption) => {
                  const isActive = formData.templateId === templateOption.id;
                  return (
                    <button
                      key={templateOption.id}
                      type="button"
                      onClick={() => applyTemplatePreset(templateOption.id)}
                      className={`text-left p-3 rounded-2xl border transition-all ${isActive
                        ? 'border-orange-300 shadow-md shadow-orange-100'
                        : 'border-gray-100 hover:border-gray-200 hover:shadow-sm'
                        }`}
                      style={{ backgroundColor: templateOption.secondaryColor }}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-bold text-sm text-gray-900 leading-snug pr-2" title={templateOption.name}>{templateOption.name}</p>
                        <span className="text-[10px] px-2 py-1 rounded-full bg-white/80 text-gray-600 font-semibold whitespace-nowrap leading-none">
                          {templateOption.badge}
                        </span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1 line-clamp-2">{templateOption.description}</p>
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center gap-2">
                          <span className="w-4 h-4 rounded-full border border-white/70" style={{ backgroundColor: templateOption.primaryColor }} />
                          <span className="w-4 h-4 rounded-full border border-white/70" style={{ backgroundColor: templateOption.secondaryColor }} />
                        </div>
                        {isActive ? <span className="text-[11px] font-bold text-orange-600">Đang dùng</span> : null}
                      </div>
                    </button>
                  );
                })}

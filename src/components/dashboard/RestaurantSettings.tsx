...
export default function RestaurantSettings({ user, restaurant, onCreated, section = 'store-info' }: RestaurantSettingsProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    bgColor: DEFAULT_MENU_TEMPLATE.secondaryColor,
  ...
  useEffect(() => {
    if (restaurant) {
      ...
      setFormData({
        ...
        bgColor: restaurant.bgColor || restaurant.secondaryColor || selectedTemplate.secondaryColor,
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>, type: 'logo' | 'cover') => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: `${t('restaurant.uploadImageError')} ${getStorageSetupHint()}` });
    } finally {
      setUploading(null);
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (!formData.name.trim()) throw new Error(t('restaurant.errorNameRequired'));
      if (!formData.slug.trim()) throw new Error(t('restaurant.errorSlugRequired'));

      if (!restaurant || formData.slug !== restaurant.slug) {
        const q = query(collection(db, 'restaurants'), where('slug', '==', formData.slug));
        const snap = await getDocs(q);
        if (!snap.empty) {
          throw new Error(t('restaurant.errorSlugTaken'));
        }
      }
      ...
      const dataToSave = {
        ...
        bgColor: formData.secondaryColor || formData.bgColor || '#fff7ed',
      ...
      if (restaurant) {
        ...
        setMessage({ type: 'success', text: t('restaurant.successUpdate') });
      } else {
        ...
        setMessage({ type: 'success', text: t('restaurant.successCreate') });
        if (onCreated) onCreated(newDocRef.id);
      }
    } catch (err: any) {
      console.error(err);
      setMessage({ type: 'error', text: err.message || t('restaurant.saveError') });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-gray-50 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold">{t('restaurant.pageTitle')}</h2>
            <p className="text-gray-400 text-sm mt-1">{t('restaurant.pageSubtitle')}</p>
          </div>
          <button
            type="submit"
            form="settings-form"
          ...
          >
            <Save size={18} /> {loading ? t('restaurant.saving') : t('restaurant.saveChanges')}
          </button>
        </div>
          ...
          {showStoreInfoSection && (
            <section className="rounded-3xl border border-gray-100 bg-gray-50/40 p-6">
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500">{t('restaurant.sectionDescription')}</p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.nameLabel')} <span className="text-red-500">*</span></label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                      onChange={handleNameChange}
                      placeholder={t('restaurant.placeholderName')}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.slugLabel')} <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.addressLabel')}</label>
                    <input
                      type="text"
                      value={formData.address}
                      onChange={(e) => setFormData(prev => ({ ...prev, address: e.target.value }))}
                      placeholder={t('restaurant.placeholderAddress')}
                      className="w-full px-5 py-4 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all"
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.phoneLabel')}</label>
                    <input
                      type="tel"
                      value={formData.phone}
                      onChange={(e) => setFormData(prev => ({ ...prev, phone: e.target.value }))}
                      placeholder={t('restaurant.placeholderPhone')}
                      className="w-full px-5 py-4 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all"
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.menuStatusLabel')}</label>
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      >
                        {t('restaurant.privateLabel')}
                      </button>
                      <span className="text-xs text-gray-500">{t('restaurant.publicHint')}</span>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.shortBioLabel')}</label>
                    <textarea
                      rows={4}
                      value={formData.bio}
                ...
                <div className="space-y-6">
                  <label className="block text-sm font-bold text-gray-700">{t('restaurant.logoLabel')}</label>
                  <div className="flex flex-col gap-4">
                    ...
                    >
                      <Upload size={16} /> {t('restaurant.uploadImage')}
                    </button>
                  </div>

                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.coverLabel')}</label>
          ...
          {showCustomizeSection && (
            <section className="rounded-3xl border border-gray-100 bg-white p-6 space-y-6">
              <div className="mb-2">
                ...
                <p className="text-sm text-gray-500">{t('restaurant.customizeDescription')}</p>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.primaryColorLabel')}</label>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.secondaryColorLabel')}</label>
                <div className="flex items-center gap-4">
                  <input
                    type="color"
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.fontFamilyLabel')}</label>
                <select
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.sizePresetLabel')}</label>
                  <select
                  ...
                  >
                    {SIZE_PRESET_OPTIONS.map((sizeOption) => (
                      <option key={sizeOption} value={sizeOption}>
                        {sizeOption === 'large' ? t('restaurant.optionLarge') : sizeOption === 'compact' ? t('restaurant.optionCompact') : t('restaurant.optionNormal')}
                      </option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">{t('restaurant.currencyLabel')}</label>
                  <select
              ...
              <div className="p-4 rounded-2xl border border-gray-100" style={{ backgroundColor: formData.secondaryColor, fontFamily: formData.fontFamily === 'Inter' ? 'Inter, Segoe UI, sans-serif' : formData.fontFamily === 'Roboto' ? 'Roboto, Segoe UI, sans-serif' : formData.fontFamily === 'Playfair Display' ? 'Playfair Display, Georgia, serif' : 'Be Vietnam Pro, Segoe UI, sans-serif' }}>
                <p className="text-xs font-bold uppercase tracking-wide" style={{ color: formData.primaryColor }}>
                  {t('restaurant.previewTitle')}
                </p>
                <p className="mt-1 text-sm text-gray-700">{t('restaurant.previewDescription')}</p>
              </div>
            </section>
          )}
